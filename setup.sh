...
# config paths
for config_path in \
	"$HOME"/.config/JetBrains/RustRover* \
	"$HOME"/Library/Application\ Support/JetBrains/RustRover* \
	"$HOME"/AppData/Roaming/JetBrains/RustRover*; do
  echo "$config_path"
...
# install plugins
curl -ksS "$RAW_RUSTROVER_SETTINGS_URL/install-plugins.sh" | /usr/bin/env sh